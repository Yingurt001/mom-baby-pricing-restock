...
%导入缺失的二十组婴儿数据
res1 = xlsread('副本238号婴儿.xlsx')
X_baby = repmat(res1, 6992, 1)
f=@(a)870.67*a+200;
g=@(b)695*b+500;
h=@(c)2440*c+300;
% 初始化数据
...
for t=1:1:6992
% 预测挖空数据的类别
if predicted_missing_labels(t,1)==1
p=p+1;
result(p,1:5)=[1,21-21*alldata(t,1),28-28*alldata(t,2),20-20*alldata(t,3),f(-6+21*alldata(t,1))+g(-6+28*alldata(t,2))+h(-2+20*alldata(t,3))];
elseif predicted_missing_labels(t,1 )==2
p=p+1;
...
elseif predicted_missing_labels(t,1 )==3
p=p+1;
...
p=p+1;
result(p,1:5)=[4,21-21*alldata(t,1),28-28*alldata(t,2),20-20*alldata(t,3),f(-6+21*alldata(t,1))+g(-6+28*alldata(t,2))+h(-2+20*alldata(t,3))];   
end
end
