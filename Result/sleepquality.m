...
%然后我们用得到的回归模型去预测20组婴儿的睡眠质量
%

clc;clear
% train是训练集的自变量，即母亲的身体特征和心理特征数据
% train_result是训练集的因变量，即婴儿的睡眠质量
% test是测试集的自变量，即母亲的身体特征和心理特征数据
% train_result是测试集的因变量，即婴儿的行为特征，1为矛盾，2为中等，3为安静
% 导入数据训练集和测试集的数据
res = xlsread('睡眠质量与身体心理指标.xlsx');
train = res(1:300, 6:8);%取前二百行的数据,行为样本数据，列为指标
train_result = res(1:300, 14);%第十四列为婴儿的睡眠质量
test = res(301:end, 6:8);%取二百行后的数据,心理分数数据，列为测试集的指标
test_result = res(301:end, 14);%第十四列为将要对比的婴儿的行为特征答案

% 使用TreeBagger内嵌函数建立随机森林模型，决策树为100棵
